...
function updateKanban() {
  let kanban_column = '.kanban_column';
  let kanban_body = $('.kanban_body');
  $(kanban_column).sortable({
    connectWith: kanban_column,
    update: function (event, ui) {
      let new_value = ui.item.prev().parent().attr('id').toString(); // FIXME: Feels inaccurate
      let object_id = ui.item[0].id;
      let url = object_id.toString();
      let kanban_group = kanban_body.data('kanban_group');
      let model = kanban_body.data('model');
      let data = '{ "' + model + '":{ "' + kanban_group + '":"' + new_value + '"} }';
      $.ajax({
        type: 'PUT',
